...
@software: pycharm
@file: BayesLaplaceSmoothing.py
@time: 2020/1/29 20:09
@desc
"""
import pandas as pd
import numpy as np
...
class BayesLaplaceClassifier(object):
    ...
    def _priori_probability(self):
        ...
        classify_map = map(lambda index: (classifications[index],
                                          (counts[index]+lambda_num) / (total_number+k*lambda_num),
                                          counts[index]),
        ...
        for item in range(len(self._train_data)):
            ...
            for element in features:
                if element == classification_name:
                    continue
                sj = np.unique(self._train_data[element])
                denominator = classify_counts[classify_name.index(classification)]+len(sj)*lambda_num
                ...
                combination_counts.append(1+lambda_num)
                combination_probability.append((1+lambda_num) / denominator)
...
if __name__ == '__main__':
    solution = BayesLaplaceClassifier()
    solution.main()
